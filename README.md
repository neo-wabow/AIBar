...
Claude Code 的官方額度只能來自 Claude Code `statusLine` 產生的 `~/.ai-usage/claude-status/*.json`。如果這個來源不存在、沒有 `rate_limits`，或該視窗的 reset window 已經過期，AIBar 必須顯示未同步，不能用其他本機資料補百分比。若 Claude Code idle 導致 statusline 暫停寫檔，但快照內仍有未過期的官方 reset window，AIBar 可以顯示最後一次官方同步的百分比，並標示最後同步時間。Reload 只會重讀本機快照；如果 Claude Code 沒有產生新的官方快照，Reload 不會創造新的百分比。唯一例外是 Claude Code 本機 transcript 明確記錄 429 `rate_limit` 且包含 reset 時間時，AIBar 可顯示 5 小時剩餘 `0%` 與該 reset 時間。

特別禁止把 `~/Library/Application Support/Claude/plan-usage-history.json`、Claude Desktop cache、IndexedDB / Session Storage 解析結果，或任何推算值顯示成 Claude Code 官方 quota。這類資料曾造成 Claude Desktop cache 被誤標成 Claude Code 百分比。

「加入監看」的帳號走的雲端路徑即是遵守這個原則接官方 usage：用該帳號本機儲存的登入憑證查詢即時額度,查詢失敗(如暫時限流)時保留最後一次成功值並在卡片標示,而不是用推算值頂替。access token 過期才會刷新並寫回;其餘時間只讀。
